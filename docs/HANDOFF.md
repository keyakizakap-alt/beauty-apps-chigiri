...
### 3-1. 商品データ92点の公式ページ突合（最優先・進行中 7/92）
...
記入時の注意（2026-08-12 の実作業で実際に起きたこと）:
- `drop` は「取り扱いをやめる」。値を直したいときは **`fix`**。間違えると更新されず削除候補になる
- 確認日は `2026-08-12` の形式のみ。`8月12日` は弾かれる
- 公式URLのホストが新しくなったら `data/merchants.json` への追加が必要
- 「正しい〜」列を空欄にすると現状維持。つまり**確認していない項目も「確認済み」の印が付く**ので、
  価格まで見たかどうかは備考欄に残すとよい

**次の一手**: `npm run verify:export -- skincare` で残りのスキンケア39点を書き出し、
同じ手順で埋める。分野ごとに区切ると1回あたりの作業量が現実的になります。
