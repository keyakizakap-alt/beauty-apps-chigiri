...
import ChigiriMark from "@/components/ChigiriMark";
...
export default function DatabasePage() {
  ...
  return (
    <main className="mx-auto w-full max-w-2xl px-5 pb-16 pt-8">
      <nav className="mb-6 flex items-center gap-2 text-xs text-inkSoft">
      ...
      <header>
        ...
        <h1 className="mt-3 text-2xl font-semibold text-forest">
          公式情報データベース
        </h1>
      ...
      <dl className="mt-5 grid grid-cols-3 gap-2">
        {[
          ...
          <div key={m.label} className="chigiri-card px-3 py-3 text-center">
            <dt className="text-[11px] text-inkSoft">{m.label}</dt>
            <dd className="mt-0.5 text-lg font-semibold text-forest">{m.value}</dd>
          </div>
        ))}
      </dl>

      <p className="mt-2 rounded-soft border border-clay/30 bg-claySoft/60 px-3 py-2.5 text-[11px] leading-relaxed text-ink">
      ...
      <section className="mt-6 space-y-3">
        <div>
          ...
          <input
            ...
            className="chigiri-tap w-full rounded-soft border border-line bg-cream px-4 text-sm outline-none focus:border-forest"
          />
        </div>
      ...
      <p className="mt-5 text-xs text-inkSoft" aria-live="polite">
      ...
      <ul className="mt-3 space-y-2.5">
            ...
            <li key={p.id} className="chigiri-card overflow-hidden">
              ...
              >
                <span className="min-w-0 flex-1">
                  <span className="block text-[11px] text-inkSoft">{p.brand}</span>
                ...
                <span aria-hidden className="mt-1 shrink-0 text-inkSoft">
                  {open ? "−" : "＋"}
                </span>
              </button>
              ...
              {open && (
                <div className="border-t border-line px-4 py-4">
                  ...
                  <a
                    ...
                    className="chigiri-tap mt-3 inline-flex items-center rounded-full border border-forest px-4 text-xs text-forest"
                  >
                    公式サイトを開く
                  </a>
        ...
        {filtered.length === 0 && (
          <li className="rounded-card border border-dashed border-line px-4 py-10 text-center text-sm text-inkSoft">
...
function Tag({ children }: { children: React.ReactNode }) {
  return (
    <span className="rounded-full bg-sageSoft px-2 py-0.5 text-[10px] text-forest">
      {children}
    </span>
  );
}
